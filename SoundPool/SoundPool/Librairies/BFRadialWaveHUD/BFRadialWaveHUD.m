...
+ (BFRadialWaveHUD *)sharedInstance {
	static BFRadialWaveHUD *hud = nil;
	static dispatch_once_t onceToken;
	dispatch_once(&onceToken, ^{
	    hud = [[BFRadialWaveHUD alloc] initWithView:nil fullScreen:YES circles:25 circleColor:nil mode:BFRadialWaveHUDMode_Default strokeWidth:2.3];
...
+ (void)showInView:(UIView *)currentView withMessage:(NSString *)message {
	[BFRadialWaveHUD sharedInstance].frame = currentView.bounds;
	[BFRadialWaveHUD sharedInstance].container = currentView;
	[[BFRadialWaveHUD sharedInstance] showWithMessage:message];
}
...
- (void)setUpWithContainer:(UIView *)containerView
	...
	self.atTheDisco = NO;
	// Public:
	self.messageFont = [UIFont fontWithName:@"AppleSDGothicNeo-Medium" size:18.f];
